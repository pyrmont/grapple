...
(defn use-fixtures
  ```
  Set up fixtures for test suite.
  ```
  [kind & args]
  (def fixts (reduce (fn [f1 f2]
                       (if (and (not= :function (type f2))
                                (not= :cfunction (type f2)))
                         (error "fixtures must be functions"))
                         (fn [g] (f1 (fn [] (f2 g)))))
                     fixts-each
                     args))
  (case kind
    :each
    (set fixts-each fixts)

    (error "only :each is implemented")))
