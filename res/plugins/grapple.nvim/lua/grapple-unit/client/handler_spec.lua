...
mock_str = {split = _6_}
local mock_ui
local function _7_(bufnr, file_path, line)
  return table.insert(ui_calls, {op = "add", bufnr = bufnr, ["file-path"] = file_path, line = line})
end
local function _8_(file_path, line)
  return table.insert(ui_calls, {op = "remove", ["file-path"] = file_path, line = line})
end
local function _9_()
  return table.insert(ui_calls, {op = "clear"})
end
local function _10_(bufnr, file_path, line)
...
mock_ui = {["add-breakpoint-sign"] = _7_, ["remove-breakpoint-sign"] = _8_, ["clear-breakpoint-signs"] = _9_, ["show-debug-indicators"] = _10_, ["hide-debug-indicators"] = _11_}
...
local function _12_()
  ...
  it("handles sess.new messages", _14_)
  local function _15_()
    local msg = {op = "env.eval", val = nil}
    handler["handle-message"](msg, nil)
    return assert.equals(0, #log_calls)
  end
  it("handles env.eval messages with nil value", _15_)
  local function _16_()
  ...
  it("handles env.eval messages with stdout", _16_)
  local function _17_()
  ...
  it("handles env.eval messages with stderr", _17_)
  local function _18_()
  ...
  it("handles env.eval messages with return value", _18_)
  local function _19_()
  ...
  it("handles env.eval messages with note", _19_)
  local function _20_()
  ...
  it("handles env.load messages like env.eval", _20_)
  local function _21_()
    local msg = {op = "dbg.brk.add", tag = "ret", ["janet/bp"] = "./test.janet:10"}
    local opts = {bufnr = 1, ["file-path"] = "./test.janet", line = 10}
    handler["handle-message"](msg, opts)
    assert.equals(1, #log_calls)
    assert.equals("debug", log_calls[1].sec)
    local lines = log_calls[1].lines
    return assert.equals("Breakpoint added: ./test.janet:10", lines[1])
  end
  it("handles dbg.brk.add response", _21_)
  local function _22_()
    local msg = {op = "dbg.brk.rem", tag = "ret", ["janet/bp"] = "./test.janet:10"}
    local opts = {["file-path"] = "./test.janet", line = 10}
    handler["handle-message"](msg, opts)
    assert.equals(1, #log_calls)
    assert.equals("debug", log_calls[1].sec)
    local lines = log_calls[1].lines
    return assert.equals("Breakpoint removed: ./test.janet:10", lines[1])
  end
  it("handles dbg.brk.rem response", _22_)
  local function _23_()
  ...
  it("handles dbg.brk.clr response", _23_)
  local function _24_()
  ...
  it("handles dbg.insp.stk response with stack frames", _24_)
  local function _25_()
  ...
  it("handles dbg.insp.stk response without stack frames", _25_)
  local function _26_()
  ...
  it("handles debug signal within env.eval", _26_)
  local function _27_()
  ...
  it("handles error messages", _27_)
  local function _28_()
  ...
  it("handles unrecognized messages", _28_)
  local function _29_()
    handler["handle-message"](nil, nil)
    return assert.equals(0, #log_calls)
  end
  return it("handles nil messages gracefully", _29_)
end
return describe("handle-message", _12_)
