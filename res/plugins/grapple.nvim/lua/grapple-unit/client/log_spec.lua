...
local function _5_()
  ...
  local function _14_()
    local buf_num = client_log.buf()
    local ns = vim.api.nvim_create_namespace("grapple-log")
    client_log.append("note", {"Re-evaluating dependents of x: y, z"}, {})
    return assert.is_true((count_extmarks_with_hl(buf_num, ns, "Special") > 0))
  end
  it("highlights note content with Special", _14_)
  local function _15_()
    local buf_num = client_log.buf()
    local ns = vim.api.nvim_create_namespace("grapple-log")
  ...
  it("does not add highlights for input content", _15_)
  local function _16_()
    local buf_num = client_log.buf()
    local ns = vim.api.nvim_create_namespace("grapple-log")
    local title_before = count_extmarks_with_hl(buf_num, ns, "Title")
  ...
  return it("does not add highlights for result content", _16_)
end
